...
async function assignToStation() {
    const voterId = localStorage.getItem("voter_id");
    const boothId = localStorage.getItem("booth_id"); // Get booth_id from localStorage
    ...
    try {
        ...
        // Check for active stations within the booth
        const { activeStation, error } = await findActiveStation(boothId); // Pass boothId
        if (error) {
            statusDiv.innerHTML += `
        ...
        statusDiv.innerHTML += `
            <div class="alert alert-success mt-3">
                ...
                You have been assigned to station <strong>${activeStation.id}</strong> in Booth <strong>${boothId}</strong>.
                <div class="mt-3">
...
async function findActiveStation(boothId) {
    try {
        ...
        if (!stationsSnapshot.exists()) {
            return { error: "No stations found for this booth. Please contact an administrator." };
        }
